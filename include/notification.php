...
class Notification {
	protected static  $tblname = "notification";
	...
	function find_notifications($id="",$when=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE notification_id = {$id} OR notification_date = '{$when}'");
		// $cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows();
		return $row_count;
	} 

	function find_all_notifications($when=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE notification_date = '{$when}'");
		// $cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows();
		return $row_count;
	}
	 
	function single_notification($id=""){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tblname." 
