...
// comment.php
class Reply {
    protected static  $tblname = "replies";
        ...
        function listofreplies(){
            global $mydb;
            $mydb->setQuery("SELECT * FROM ".self::$tblname);
            return $cur;
        }
        function find_replies($id="",$when=""){
            global $mydb;
            $mydb->setQuery("SELECT * FROM ".self::$tblname." 
                WHERE reply_id = {$id} OR reply_create_at = '{$when}'");
            // $cur = $mydb->executeQuery();
            $row_count = $mydb->num_rows();
            return $row_count;
        } 
    
        function find_all_replies($when=""){
            global $mydb;
            $mydb->setQuery("SELECT * FROM ".self::$tblname." 
                WHERE reply_create_at = '{$when}'");
            // $cur = $mydb->executeQuery();
            $row_count = $mydb->num_rows();
            return $row_count;
        }
         
        function single_replies($id=""){
                global $mydb;
                $mydb->setQuery("SELECT * FROM ".self::$tblname." 
                    Where reply_id = '{$id}' LIMIT 1");
