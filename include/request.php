...
class Request {
	protected static  $tblname = "request_info";
	...
	function listofstudentrequest(){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname);
		// $cur = $mydb->loadResultList();
		return $cur;
	}
	function find_studentrequest($id="",$when=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE request_info_id = {$id} OR ANNOUNCEMENT_WHEN = '{$when}'");
		// $cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows();
		return $row_count;
	} 

	function find_all_studentrequest($when=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
	...
	function single_studentrequest($id=""){
        global $mydb;
        $mydb->setQuery("SELECT * FROM ".self::$tblname." 
            WHERE request_info_id = '{$id}' LIMIT 1");
